...
/**
        $('body').on('click', '#btnExcel', function (evt) {
            fn_gera_excel('tbl-fat-estados', 'nova-pagina');
        });
...
function fn_gera_excel(tabelaID, pagina) {
    ...
    } else if (pagina == 'MapaFaturamento') {

        let tableClo = tableOri.cloneNode(true);

        tableClo.querySelectorAll('tr:nth-child(n+2) td:nth-child(n+2)').forEach((a) => {

            a.querySelector('[data-noexcel]')?.remove();

            let textA = a.closest('td').querySelector('a')?.innerText;
            let textT = a.closest('td')?.innerText;

            if (textA != undefined && isNaN(parseInt(textA))) {
                a.closest('td').querySelector('a').innerText = 0;
            }

            if (textT != undefined && isNaN(parseInt(textT))) {
                a.closest('td').innerText = 0;
            }
        });

        tableToExcel(tableClo, 'Relatorio');
    }
}